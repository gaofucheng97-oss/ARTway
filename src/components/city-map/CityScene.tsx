...
function ArtworkIcon({ variant, featured }: { variant: string; featured: boolean }) {
  ...
  switch (variant) {
    case "castaway":
    case "henrietta-lacks": // standing figure
      return (
        <group>
          <Block position={[0, 0.34, 0]} scale={[0.34, 0.5, 0.28]} mat={matWhite} />
          <mesh material={accent} position={[0, 0.8, 0]}>
            <sphereGeometry args={[0.2, 12, 10]} />
          </mesh>
        </group>
      );
    case "atlas": // paired globes
      return (
        <group>
          <mesh material={accent} position={[-0.32, 0.52, 0]}>
            <sphereGeometry args={[0.3, 14, 10]} />
          </mesh>
          <mesh material={matGreen} position={[0.32, 0.52, 0]}>
            <sphereGeometry args={[0.22, 14, 10]} />
          </mesh>
          <Block position={[0, 0.23, 0]} scale={[0.95, 0.12, 0.5]} mat={matWhite} />
        </group>
      );
    case "follow-me": // mirrored panels
      return (
        <group>
          <Block position={[-0.28, 0.5, 0.1]} scale={[0.5, 1, 0.1]} mat={matWhite} />
          <Block position={[0.28, 0.42, -0.2]} scale={[0.42, 0.84, 0.1]} mat={accent} />
        </group>
      );
    ...
    case "voronoi-screen": // cellular screen
      return (
        <group>
          <Block position={[0, 0.55, 0]} scale={[0.16, 1.1, 0.12]} mat={accent} />
          <Block position={[0.32, 0.48, -0.12]} scale={[0.16, 0.96, 0.12]} mat={matWhite} />
          <Block position={[-0.32, 0.42, -0.12]} scale={[0.16, 0.84, 0.12]} mat={matWhite} />
        </group>
      );
    case "edith-and-hans": // outdoor room
      return (
        <group>
          <Block position={[-0.36, 0.34, 0]} scale={[0.14, 0.68, 0.9]} mat={matWhite} />
          <Block position={[0.36, 0.34, -0.4]} scale={[0.9, 0.68, 0.14]} mat={accent} />
          <Block position={[0, 0.16, 0.38]} scale={[0.52, 0.12, 0.14]} mat={matGreen} />
        </group>
      );
    case "uncertain-world": // mural wall
      return (
        <group>
